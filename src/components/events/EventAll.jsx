...
export const EventAll = () => {
  ...
  const fallbackEvents = [
    {
      ...
      src: event1,
      location: "MIT Campus, Cauvery Cross, Mettur Dam",
      date: "Apr 20, 2026",
      time: "10:00 AM",
      title: "Campus Orientation Program",
      description: "An introduction session for students and parents covering campus facilities, academic guidance, and student support services.",
    },
    {
      ...
      description: "Meet the faculty team and explore diploma and ITI programs, eligibility, scholarships, and career opportunities.",
    },
  ];
  ...
  const getEventImageUrl = (imageData) => {
    return getImageUrl(imageData) || event1;
  };
  ...
  const displayEvents = events.length > 0 
    ? events.map(event => ({
        src: getEventImageUrl(event.attributes?.image),
        location: event.attributes?.location || "MIT Campus, Cauvery Cross, Mettur Dam",
        date: formatDate(event.attributes?.eventDate),
        time: formatTime(event.attributes?.eventDate),
      ...
      <div className="container">
        <div className="row td_gap_y_30">
          {displayEvents.map((event, index) => (
            <div key={event.id || index} className="col-lg-6">
              <Item {...event} />
            </div>
          ))}
        </div>
