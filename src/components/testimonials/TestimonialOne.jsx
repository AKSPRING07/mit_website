...
import avatar2 from "../../assets/img/home_1/avatar/avatar_2.jpg";
import { mitProgramBySlug } from "../../data/mitPrograms";
...
export const TestimonialOne = () => {
  ...
  const defaultTestimonials = [
    { name: "Ranjit Kumar", batch: `2023 Batch - ${mitProgramBySlug.get("dme").title}`, review: "MIT-la padichithu romba nalla experience. Lab facilities semma and teachers ellam romba supportive. Ipo oru reputed company-la work panren. Thank you MIT!", rating: "4.5", avatar: avatar1 },
    { name: "Raghav S", batch: `2022 Batch - ${mitProgramBySlug.get("aiml").title}`, review: "MIT-ல் படித்ததால் எனக்கு நல்ல வேலை கிடைத்தது. ஆசிரியர்கள் மிகவும் நன்றாக பாடம் சொல்லித் தருவார்கள். Campus life-உம் மிகவும் சிறப்பாக இருந்தது!", rating: "4.5", avatar: avatar2 }
  ];

  const getTestimonialImageUrl = (imageData) => {
    return getImageUrl(imageData) || avatar1;
  };

  const displayTestimonials = testimonials.length > 0
    ? testimonials.map((item, index) => ({
        name: item.attributes?.studentName || "Student",
        batch: item.attributes?.batch || "Alumni",
        review: item.attributes?.content || "No review available",
        rating: item.attributes?.rating?.toString() || "5",
        avatar: getTestimonialImageUrl(item.attributes?.photo) || (index % 2 === 0 ? avatar1 : avatar2)
      }))
    : defaultTestimonials;

  console.log('Testimonials state:', testimonials);
  console.log('Display testimonials:', displayTestimonials);
  console.log('Number of display testimonials:', displayTestimonials.length);
      ...
      <div className="container">
        ...
        >
          ...
          <p className="td_section_subtitle td_fs_18 mb-0 td_white_color td_opacity_7">
            Hear from our successful students who have completed their
            <br />
            Diploma and ITI courses and are now building their careers
            <br />
          ...
          >
            <div className="td_testimonial_img_wrap">
              <img
                src={testimonialImg}
                alt="Testimonial"
                className="td_testimonial_img rounded-3"
              />
                ...
                <div className="td_slider_wrapper">
                  <SlickSlider {...settings}>
                    {displayTestimonials.map((testimonial, index) => (
                      <div key={index} className="td_slide">
