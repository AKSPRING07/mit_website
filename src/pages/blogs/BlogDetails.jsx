...
export const BlogDetails = ({ hideHeaderImage = false }) => {
  const { id } = useParams();
  ...
  if (loading) {
    return (
      <Layout header={1} footer={1} breadcrumbTitle="Blog Details" breadcrumbSubtitle="Loading...">
        <BlogContainer>
  ...
  if (!blog && id) {
    return (
      <Layout header={1} footer={1} breadcrumbTitle="Blog Not Found" breadcrumbSubtitle="Error">
        <BlogContainer>
          <div className="text-center" style={{ padding: '100px 0' }}>
            <h3>Blog post not found</h3>
            <Link to="/blog" className="td_btn td_style_1 td_radius_10 td_medium">
  ...
  const blogTitle = attrs?.title || "Blog Details";
  const blogContent = attrs?.content || "No content available.";
  const blogImage = getImageUrl(attrs?.image) || attrs?.imageUrl || blogDetails1;
  const blogAuthor = attrs?.author || "Admin";
  const blogDate = attrs?.publishedAt ? new Date(attrs.publishedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : "January 21, 2023";
  const blogCategory = attrs?.category || "Blog";
  const blogExcerpt = attrs?.excerpt || blogContent.substring(0, 200) + '...';
  ...
  return (
    ...
    //   header={1}
    //   footer={1}
    //   breadcrumbTitle={blogTitle}
    //   breadcrumbSubtitle={blogCategory}
    // >
      <BlogContainer>
        <div className="td_blog_details_head td_mb_40">
          {!hideHeaderImage && <img src={blogImage} alt={blogTitle} />}
          ...
          <div className="td_blog_details_head_meta">
            <div className="td_blog_details_avatar">
              ...
              <p className="mb-0 td_heading_color td_bold">
                <span className="td_normal td_opacity_5">By</span> {blogAuthor}
              </p>
            </div>
        ...
        <div className="td_post_share">
          <div className="td_categories">
            <h4 className="mb-0 td_fs_18">Category:</h4>
            <span className="td_category">
              {blogCategory}
            </span>
          </div>
        ...
        <div className="td_author_card">
          <div className="td_author_card_in">
            ...
            <div className="td_author_card_right">
              ...
              <h3 className="td_fs_20 td_semibold td_mb_10">{blogAuthor}</h3>
            </div>
          </div>
        </div>
        <div className="td_height_40 td_height_lg_40" />
